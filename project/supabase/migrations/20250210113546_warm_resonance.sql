...
CREATE OR REPLACE FUNCTION handle_message_timestamps()
RETURNS TRIGGER AS $$
BEGIN
  NEW.created_at = now();
  RETURN NEW;
...
CREATE TRIGGER set_message_timestamps
  BEFORE INSERT ON messages
  FOR EACH ROW
  EXECUTE FUNCTION handle_message_timestamps();

-- Create function to handle trial timestamps
CREATE OR REPLACE FUNCTION handle_trial_timestamps()
RETURNS TRIGGER AS $$
BEGIN
...
-- Create trigger for trials
CREATE TRIGGER set_trial_timestamps
  BEFORE INSERT OR UPDATE ON trials
  FOR EACH ROW
  EXECUTE FUNCTION handle_trial_timestamps();